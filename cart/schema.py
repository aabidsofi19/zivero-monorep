...
from shop.schema import ProductType,CategoryType,VariationType
from .cart import Cart,AbstractCart
...
class Query(graphene.ObjectType):
    ...
    def resolve_cart(self,info,**kwargs):
        ...
        print(cart_products)
        return CartType(cartLength=length,totalPrice=total_price,products=cart_products)
...
class AddToCartMutation(graphene.Mutation):
    ...
    def mutate(root,info,productdetails):
        ...
        cart.add(product_id=pd.product_id, quantity=pd.quantity,override_quantity=False,variation_id=pd.variation_id,)
        cart.save()
        # print(cart.products())
...
class UpdateCartMutation(graphene.Mutation):
    ...
    def mutate(root,info,Input):
        ...
        if Input.quantity<=0:
            ...
            cart.add(product_id=Input.product_id,override_quantity=True,quantity=Input.quantity,variation_id=Input.variation_id)
        cart=Query().resolve_cart(info)
