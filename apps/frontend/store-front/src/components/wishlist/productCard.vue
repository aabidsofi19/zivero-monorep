<template>
        <v-card class="pa-0 "  outlined tile        >
            ...
            <v-img
                ...
                :src='image'
            >  
               <v-icon class="float-right ma-5 rounded-circle white pa-2 cursor-pointer" @click.stop="removeProduct()">
                   mdi-close
               </v-icon>
            </v-img>
            ...
            <v-container  class="productDetails  d-flex flex-column pa-0  ma-0 " >
                ...
                <div outlined tile class="">
                    <v-btn @click="openDialog"  block class="secondary--text font-weight-bold" flat tile color="white" elevation="0">
                        Add to bag
                    </v-btn>
                </div>
            </v-container>

            <selectVariation :productId="id" :open="addToCartDialog" @close="closeDialog" :loading="loadingVariation"/>
       </v-card>
</template>
