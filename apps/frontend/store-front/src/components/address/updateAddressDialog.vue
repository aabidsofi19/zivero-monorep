<template>
   ...
   <v-dialog v-model="propModel" width="450px">
        <div class="white">
           <div class="py-6 px-4 text-h5 font-weight-bold d-flex justify-space-between "> 
           ...
           <addressForm class="form" ref="addressForm" update="true" ></addressForm>
            <div class="bottom-nav rounded elevation-4">
                  <v-btn color="green white--text py-6" width="100%" tile @click="submit()"> Update Address</v-btn>  
            </div>
        </div>
   </v-dialog>
...
export default{
   ...
   computed: {
      propModel: {
         ...
         set (value) { this.$emit('updateDialog', value) },
      },
   },
   methods:{
