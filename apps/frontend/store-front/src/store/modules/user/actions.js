...
import {LOGIN_EMAIL, REGISTER_USER,REFRESH_TOKEN,REVOKE_TOKEN,VERIFY_ACCOUNT, RESEND_ACTIVATION_EMAIL,FORGOT_PASSWORD,RESET_PASSWORD} from '../../../graphql/mutations/auth'
...
export default {
    logInWithEmail: async (
        ...
        let result= await client.mutate({mutation:LOGIN_EMAIL,variables:{
    ...
    registerUser:async ({commit},authDetails)=>{
        ...
        let result= await client.mutate({mutation:REGISTER_USER,variables:authDetails})
    ...
    async logOut ({ commit,}) {
        ...
        commit('app/SET_LOADING',true,{root:true})
        let {errors,loading} =await client.mutate({mutation:REVOKE_TOKEN,variables:{refreshtoken:token}})
    ...
    async refreshToken({commit}){
        ...
        let result= await client.mutate({mutation:REFRESH_TOKEN,variables:{refreshToken:refreshToken}})
    ...
    async confirmEmail(_,code){
        let result= await client.mutate({mutation:VERIFY_ACCOUNT,variables:{token:code}})
