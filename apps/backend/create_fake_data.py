...
def create_product_with_all_variations(data:ProductData):
    ...
    product = Product(name=data.name , brand=data.brand , description=data.description,category=category ,gender=data.gender)
    variations=[]
    for v in data.variations:
        variation = Variation(price=v.price,images=v.images,discount_percent=v.discount)
        variants=[]
...
def create_categories(categories:List[Category]):
    for c in categories :
        try:
            Category(name=c.name,image=c.image,genders=c.genders,brands=c.brands).save()
        except Exception as e :
...
def create(category,name,gender):
    ...
    for image in images :
        ...
        print(brand)
        tshirt = ProductData(name=name ,gender=gender,brand=Brand.objects.get(name=brand).id ,category=Category.objects.get(name=category).id, variations=variations,description="best in class")
        create_product_with_all_variations(tshirt)
        print("created")
...
kurta_images=["https://assets.myntassets.com/h_1440,q_90,w_1080/v1/assets/images/16199238/2021/11/22/c9f9d1a9-e46d-4653-9d80-6e9632ad66df1637572572660EnchantedDrapesMenMaroonThreadWorkHandloomKurta1.jpg"
...
for i in range(2000):
    ...
    create_product('kurtas',"Men Maroon Woven Design Kurta","MEN",kurta_images,random.choice(kurta_brands),discount,price)
create_product('tshirt',"New tshirt" ,"MEN",kurta_images,random.choice(brands),10,20000)