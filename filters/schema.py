...
def update_filter(filter):
    ...
    if in_genders and in_categories :
        ...
        print(categories_)
        product_brands = { brand  for category  in categories_ for brand in  category.brands }
    ...
    # if len(filter) >= 2:
    #     return {"categories": category_names, "brands": product_brands, "genders": genders}
    # # if in_genders and in_categories:
    # #     cats= Category.objects(name__in=in_categories)
    # #     product_brands=[p for p in Product.objects(
    # #
    # #          category__in=cats ).distinct('brand')]

    # if filter.get("genders"):
    #     #print("genders")
    #     # update
    #     categories = [c for c in Category.objects(
    #         gender__in=filter["genders"])]
    #     category_names = [c.name for c in categories]
    #     categories_ = [c.id for c in categories]
    #     product_brands = [p for p in Product.objects(
    #         category__in=categories_).distinct('brand')]
    # elif filter.get("categories"):
    #     categories_ = [c
    #                    for c in Category.objects(name__in=filter["categories"])]
    #     genders = []
    ...
    #     product_brands = [p for p in Product.objects(
    #         category__in=categories_).distinct('brand')]
    # elif filter.get("brands"):
    #     categories = [c for c in Product.objects(
    #         brand__in=filter["brands"]).distinct("category")]
    #     category_names = [c.name for c in categories]
    ...
    #         if c.gender not in genders:
    #             genders.append(c.gender)
    #     #print(categories, genders)

    # data = {"categories": category_names,
    #         "brands": product_brands, "genders": genders}
    # #print("data")
    # #print(data)
    # return data
    # #variantFilters = Product.objects(category=filter["category"]).distinct("variant")
