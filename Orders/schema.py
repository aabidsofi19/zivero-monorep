...
from shop.schema import ProductType,VariationType
...
class Query(graphene.ObjectType):
    ...
    order = relay.node.Field(OrderNode,id=graphene.String(),get_current=graphene.Boolean())
    ...
    def resolve_order_items(self,info,order_id,**kwargs):
        user= info.context.user
        if user.is_customer:
            order_items=OrderItem.objects.filter(order__id=order_id,Customer__user=user)
            return order_items
    ...
    def mutate(root,info,input):
        ...
        for item in cart.products():
            ...
            order_item=OrderItem(
                    order=order,product_id=str(product_id),Amount=price,
                    Address=address,Customer=customer,Quantity=item.quantity,
